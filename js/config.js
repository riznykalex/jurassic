...
export const SPECIES = {
  human: {
    label: 'Ваше плем\'я', color: '#4da6ff', diet: 'hunter', cap: 12, spawnChance: 0.00005,
    // spawnChance: раніше було 0 (плем'я не могло поповнюватись само, лише
    // спавном на старті гри) - тепер є невеликий шанс, приблизно в 30 разів
    // рідше за tribe_bot (0.0015, найближчий за архетипом вид), щоб
    // повністю вимерле плем'я мало хоч якийсь (рідкісний) шанс відновитись.
...
export const BALANCE = {
  GAME_SPEED: 1,               // ГЛОБАЛЬНИЙ множник часу симуляції: <1 - усе (рух, бій, energy,
                                // регенерація) сповільнюється РАЗОМ; >1 - прискорюється. Масштабує
                                // ефективний dt у main.js, тож зачіпає буквально все, що прив'язане
                                // до dt. НЕ впливає на анімацію спрайтів (вона на wall-clock часі).
  ENERGY_PER_PIXEL: 0.0022,
  SPEED_MULTIPLIER: 0.5,       // швидкість РУХУ окремо від GAME_SPEED (якщо треба сповільнити
                                // лише ходьбу, а бій/енергію лишити як є)
  SLEEP_REGEN: 0.012,
